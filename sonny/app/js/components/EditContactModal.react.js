...
var EditContactModal = React.createClass({
	...
	_saveContact: function(e) {
		...
		const contact = ['id', 'name', 'phone', 'email'].reduce((contact, key) => {
		...
		fetch(`http://localhost:3310/miami/contact/${contact.id}`, {
			method: 'PUT',
			body: JSON.stringify({
				firstName,
				lastName,
	...
	_removeContact: function(e) {
		...
		fetch(`http://localhost:3310/miami/contact/${removeId}`, {
			method: 'DELETE'
		}).then(() => {
